...
| `packages/`   | `ui` y `plugin-sdk` (TypeScript)                                             |

Los plugins **no** viven aquí: se desarrollan en `plugins/` (raíz) o en repos propios. La app solo
contiene el runtime que los instala y ejecuta (`crates/ft-plugins`) y el SDK que define su
contrato (`packages/plugin-sdk`).
